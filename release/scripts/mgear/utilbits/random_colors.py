...
class RandomColorTool(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        self.remove_btn.setToolTip(
            "从选定对象移除RandomColor材质。如果勾选\"应用到全部\"，"
            "则恢复所有跟踪对象的原始材质；否则只恢复选中的跟踪对象。"
        )
    ...
    def apply_random_colors(self):
        ...
        if self.apply_to_all_cb.isChecked():
            ...
            empty_msg = "场景中没有网格或NURBS对象。"
        else:
            transforms = get_selected_colorable_transforms()
            empty_msg = "未选择网格或NURBS对象！"
    ...
    def on_remove(self):
        """移除随机颜色 - 从所有跟踪对象或选中对象恢复原始材质。

        当"应用到全部"被勾选时，恢复每个跟踪对象的原始材质。
        否则，只恢复当前选中的跟踪对象。
        """
        if self.apply_to_all_cb.isChecked():
            ...
            if not selected:
                self.status_label.setText("未选择网格或NURBS对象！")
                return
            count = remove_colors(selected)
            empty_msg = "选中的对象未被跟踪。"
        if count:
            self.status_label.setText(
                f"已从 {count} 个对象移除随机颜色"
            )
        else:
            self.status_label.setText(empty_msg)
